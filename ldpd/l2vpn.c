...
l2vpn_pw_mpw_bridge(struct zapi_pw_membership *zpw)
{
    ...
    //log_debug("%s: Now we are in ldp daemon and we have access to l2vpn ifname: %s, if_index: %u \n",  __func__, zpw->ifname, zpw->ifindex);
    ...
    //log_debug("%s:   ifname: %s,   bridge_name: %s", __func__, pw->ifname, pw->l2vpn->br_ifname);
    //sprintf(cmd, "sudo vppctl set interface l2 bridge %s  %u", pw->ifname, pw->l2vpn->br_ifindex);
    //+6 because bridge has 6 letters
    //Split Horizon Group (SHG) for mpls tunnels are set to 1, so that we can avoid loop in full mesh scenario 
    sprintf(cmd, "sudo vppctl set interface l2 bridge %s  %s %d", zpw->vpp_ifname, ((pw->l2vpn->br_ifname)+6),pw->shgid);
    log_debug("%s\n", cmd);
    system(cmd);
    
    //sprintf(cmd, "sudo vppctl mpls local-label add eos %u via l2-input-on %s", zpw->local_label, pw->ifname);
    //log_debug("%s\n", cmd);
